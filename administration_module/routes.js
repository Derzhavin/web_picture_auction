    ...
    setTimeout(() => {
        ...
        for(art of arts) {
            if (art.artName === req.body.artName) {
                res.json({msg: 'Art exists!', status: 400});
                return;
            }
        }
    ...
    setTimeout(() => {
        ...
        for(participant of participants) {
            if (participant.username === req.body.username) {
                res.json({msg: 'Participant exists!', status: 400});
                return;
            }
        }
...
function sendSuccess(res, entity, operation) {
    res.status(200);
    return res.json({msg:`${entity} was ${operation} successfully!`, status: 200});
}

function sendFailure(res, entity, operation) {
    res.status(400);
    return res.json({msg:`Failed to ${operation} ${entity}!`, status: 400});
}
